...
__export(src_exports, {
  ...
  actionNamesByResource: () => actionNamesByResource,
  addActionChild: () => addActionChild,
  addRule: () => addRule,
  asGroupRoot: () => asGroupRoot,
  asRoot: () => asRoot,
  branchFields: () => branchFields,
  buildActionRoot: () => buildActionRoot,
  buildRoot: () => buildRoot,
  childrenOfAction: () => childrenOfAction,
  consumedTopFields: () => consumedTopFields,
  decorationSurfaceOptions: () => decorationSurfaceOptions,
  defaultActionRule: () => defaultActionRule,
  ...
  parseSavedRule: () => parseSavedRule,
  relabelRelations: () => relabelRelations,
  removeActionNode: () => removeActionNode,
  removeNode: () => removeNode,
  removeSchemaAction: () => removeSchemaAction,
  resolve: () => resolve,
  ...
  stringifySavedRule: () => stringifySavedRule,
  stripMeta: () => stripMeta,
  switchGroupOperator: () => switchGroupOperator,
  trimEmptyGroups: () => trimEmptyGroups,
  unwrapCompound: () => unwrapCompound,
  useActionRuleBuilder: () => useActionRuleBuilder,
  useFacetFields: () => useFacetFields,
  useFilteredCollection: () => useFilteredCollection,
  useLensValuePicker: () => useLensValuePicker,
  usePermissionBuilder: () => usePermissionBuilder,
  useRuleBuilder: () => useRuleBuilder,
  useTransitionBuilder: () => useTransitionBuilder,
...
var isSelectorClause = (facet, cond) => {
...
var leadingIdentityCount = (facet, node) => {
...
var matchFacet = (lens, decoration, node) => {
  ...
  for (const facet of decoration.facets) {
    ...
    if (lead.length === 0) {
      ...
      const applies = !resolved.elementLeaf || conds.some(isLeafOn) || tailRows.some(isLeafOn) || conds.some((c) => isSelectorClause(facet, c));
...
var stampFacetIds = (condition, lens, decoration) => {
  ...
  if ((key !== void 0 || "arrayOperator" in next) && next.__facetId === void 0) {
    ...
    if (facet) {
      if (!isPreset(facet))
        next = normalizeFacetShape(facet, next);
      next.__facetId = facetId(facet);
    }
  }
  return next;
};
var normalizeFacetShape = (facet, node) => {
  const lead = whereConditions(facet.where);
  ...
  const canonicalize = (conds) => {
    ...
    for (const field of selectorFields) {
      const at = rest.findIndex(
        (c) => isSelectorClause(facet, c) && c.field === field
      );
    ...
    return unchanged ? void 0 : next;
  };
  const rec = node;
  if (Array.isArray(rec.all)) {
    const next = canonicalize(rec.all);
    return next ? { ...rec, all: next } : node;
  }
  const reorder = (r) => {
    ...
    if (!cs) return r;
    if (cs.length === 1 && cs[0] && typeof cs[0] === "object" && "arrayOperator" in cs[0]) {
      const inner = reorder(cs[0]);
      return inner === cs[0] ? r : { ...r, condition: { ...cond, all: [inner] } };
    }
...
var writeSelectorClause = (facet, condition, field, value) => {
  ...
  if (value === null) {
    ...
    if (key === "all" && children.length === 1 && only && groupChildren2(only) !== void 0 && whereConditions(facet.where).length === 0 && !children.some((c) => isSelectorClause(facet, c)))
      return only;
    return { ...rec, [key]: children };
  }
  if (at >= 0) {
    children[at] = { ...children[at], value };
    return { ...rec, [key]: children };
  }
  const clause = { field, operator: "equals", value };
  ...
  while (block < children.length) {
    ...
    const nextField = String(next.field);
...
var buildLeaf = (node, path, depth, ctx, scope) => {
  ...
  const leafMatch = ctx.decoration && ctx.anchorLens === scope.lens ? matchFacet(ctx.anchorLens, ctx.decoration, node) : void 0;
  ...
  return {
    ...
    valid: (0, import_json_rules3.checkRuleAgainstLens)(node, scope.lens).ok,
    remove
  };
};
var buildArray = (node, path, depth, ctx, scope) => {
  ...
  const matchedFacet = !isAggregate && ctx.decoration && ctx.anchorLens === scope.lens ? matchFacet(ctx.anchorLens, ctx.decoration, node) : void 0;
  const overrideLeaf = matchedFacet ? facetElementLeaf(ctx.anchorLens, matchedFacet, ctx.surfaceOpts) : void 0;
  const relScope = rel ? (() => {
    const relLens = (0, import_json_rules3.exposedSurface)(
      (0, import_json_rules3.createLens)({
    ...
    return {
      lens: relLens,
      fields: overrideLeaf ? relFields.map((f) => f.name === overrideLeaf.name ? { ...f, ...overrideLeaf } : f) : relFields
    };
  })() : scope;
  let selectorClauseLeaves;
  const buildSub = (key) => {
    ...
    if (key === "condition" && matchedFacet) {
      ...
      if (lead > 0 && kids.length === lead + 1 && tail && isGroupNode(tail)) {
        ...
        selectorClauseLeaves = kids.slice(whereLead, lead).map((clause, i) => buildLeaf(clause, [whereLead + i], depth + 1, subCtx, relScope));
        return { ...buildGroup(tail, [lead], depth + 1, subCtx, relScope), remove: void 0 };
      }
    }
  ...
  return {
    ...
    setSelectorClause: matchedFacet?.selectors?.length && rel && !isAggregate ? (selectorField, value) => {
    ...
    valid: (0, import_json_rules3.checkRuleAgainstLens)(node, scope.lens).ok && (aggregateValidation?.ok ?? true),
...
var buildGroup = (node, path, depth, ctx, scope) => {
  const matched = ctx.decoration && ctx.anchorLens === scope.lens ? matchFacet(ctx.anchorLens, ctx.decoration, node) : void 0;
  ...
  const groupLabel = groupHoist?.label ?? (path.length === 0 ? modelDecor(ctx.decoration, ctx.anchorLens.mapName, ctx.anchorLens.model).label : void 0);
...
var import_json_rules5 = require("@inixiative/json-rules");
...
var useRuleBuilder = (opts) => {
  ...
  const anchorFields = (0, import_react2.useMemo)(() => {
  ...
  const [tree, setTree] = (0, import_react2.useState)(() => ingest(opts.defaultValue));
  const onChangeRef = (0, import_react2.useRef)(opts.onChange);
  onChangeRef.current = opts.onChange;
  const first = (0, import_react2.useRef)(true);
  const clean = (0, import_react2.useCallback)(
    (t) => (0, import_json_rules4.stampCoercions)(stripMeta(trimEmptyGroups(t) ?? EMPTY), lens),
    [lens]
  );
  ...
  const commit = (0, import_react2.useCallback)((next) => setTree(withIds(next)), []);
  ...
  const value = (0, import_react2.useMemo)(() => clean(tree), [tree, clean]);
  return {
    value,
    root,
    lens,
    setCondition: (c) => setTree(ingest(c)),
    validate: (target) => (0, import_json_rules4.validateRule)(value, { target }),
    describe: () => (0, import_json_rules4.describeRule)(value, lens)
  };
};
...
var useFilteredCollection = (opts) => {
  ...
  const sourceValues = (0, import_react3.useMemo)(
    () => (0, import_json_rules5.sourceValuesFromRows)(composeNarrowed(opts.source), rows, checkOptions),
    [opts.source, rows, checkOptions]
  );
  const builder = useRuleBuilder({ ...builderOpts, sourceValues });
  const stringMatch = (0, import_react3.useMemo)(
    () => ({
  ...
  const data = (0, import_react3.useMemo)(
    () => import_json_rules5.engineGlobals.with(
      { string: stringMatch },
      () => rows.filter((row) => (0, import_json_rules5.check)(builder.value, row, checkOptions) === true)
    ),
...
var useActionRuleBuilder = (opts) => {
  const lens = (0, import_react4.useMemo)(
    () => resolve(opts.source, { sourceValues: opts.sourceValues }),
    [opts.source, opts.sourceValues]
  );
  const fields = (0, import_react4.useMemo)(() => describeModelFields(lens, lens.mapName, lens.model), [lens]);
  const [tree, setTree] = (0, import_react4.useState)(() => opts.defaultValue ?? defaultActionRule());
  const onChangeRef = (0, import_react4.useRef)(opts.onChange);
  onChangeRef.current = opts.onChange;
  const first = (0, import_react4.useRef)(true);
  ...
  const commit = (0, import_react4.useCallback)((next) => setTree(next), []);
  const maps = opts.source.maps;
  const bridges = opts.source.bridges;
  const resourceFields = (0, import_react4.useCallback)(
    (res) => {
      const i = res.indexOf(":");
...
var import_json_rules6 = require("@inixiative/json-rules");
var import_react5 = require("react");
var RELATION_KINDS3 = /* @__PURE__ */ new Set(["object", "bridge"]);
var lensValuePicker = (lensOrNarrowing, opts = {}) => {
  const lens = (0, import_json_rules6.exposedSurface)(lensOrNarrowing);
...
var import_json_rules7 = require("@inixiative/json-rules");
var runSources = (lensOrNarrowing, rows) => (0, import_json_rules7.sourceQueries)(lensOrNarrowing).map((q) => {
  const matched = (rows[q.model] ?? []).filter((r) => (0, import_json_rules7.check)(q.composedWhere, r) === true);
  const seen = /* @__PURE__ */ new Set();
  const options = [];
