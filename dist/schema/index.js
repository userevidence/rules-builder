...
var writeSelectorClause = (facet, condition, field, value) => {
  ...
  if (value === null) {
    ...
    if (key === "all" && children.length === 1 && only && groupChildren(only) !== void 0 && whereConditions(facet.where).length === 0 && !children.some((c) => isSelectorClause(facet, c)))
      return only;
...
var lensValuePicker = (lensOrNarrowing, opts = {}) => {
  const lens = exposedSurface2(lensOrNarrowing);
